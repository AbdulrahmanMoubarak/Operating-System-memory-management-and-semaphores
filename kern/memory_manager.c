...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	{
		int val = pf_add_empty_env_page(e, (uint32)va, 0);
		if(val == E_NO_PAGE_FILE_SPACE)
		{
			panic("ERROR : No enough virtual space in page file !");
		}
		va += PAGE_SIZE/4;
	}
...
void freeMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//pt_clear_page_table_entry(struct Env* ptr_env, uint32 virtual_address)
	//void pd_clear_page_dir_entry(struct Env* ptr_env, uint32 virtual_address)
	//env_page_ws_invalidate(struct Env* e, uint32 virtual_address)

	//int fr = calculate_free_frames();
	uint32 no_of_pages = size/PAGE_SIZE;

	uint32* ptr2 = (uint32*)ROUNDDOWN((uint32)virtual_address, PAGE_SIZE);
	...
	while( i < no_of_pages )
	{
		...
		uint32 *ptrPT = NULL;
		get_page_table(e->env_page_directory, (void*)va, &ptrPT);

		for(int j = 0 ; j < e->page_WS_max_size ; j++)
		{
			uint32 eFlag = env_page_ws_is_entry_empty(e, j);
			uint32 wsVa = env_page_ws_get_virtual_address(e, j);

			if(!eFlag && ROUNDDOWN(wsVa, PAGE_SIZE) == ROUNDDOWN((uint32)va, PAGE_SIZE))
			{
		...
		if(ptrPT != (uint32*)NULL)
		{
			int count = 0;
			for(int k = 0 ; k < 1024 ; k++)
			{
				uint32 entery = ptrPT[k];
			...
			{
				uint32 *ptrPT2 = NULL;
				struct Frame_Info *fi2 = NULL;//get_frame_info(e->env_page_directory, (void*)ptrPT, &ptrPT2);
				int Phy_add = (e->env_page_directory[PDX((uint32)va)]>> 12) * PAGE_SIZE;
				fi2 = to_frame_info(Phy_add);
				fi2->references = 0;
