...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	int getRet = get_share_object_ID(ownerID, shareName);
	if(getRet != E_SHARED_MEM_NOT_EXISTS)
	{
		return E_SHARED_MEM_EXISTS;
	}
	int IdRet = allocate_share_object(&MyShare);
	if(IdRet == E_NO_SHARE)
	{
		return E_NO_SHARE;
	}
	...
	uint32* ptrFrames = virtual_address;
	...
	for(uint32 i = 0 ; i < num_frames ; i++)
	{
		...
		uint32 perm = isWritable << 1;
		//cprintf("frames = %d, perms > %d\n", num_frames, (PERM_WRITEABLE&perm));
		add_frame_to_storage(shares[IdRet].framesStorage, ptr_new_frame, i);
		map_frame(myenv->env_page_directory, ptr_new_frame, (void*)ptrFrames, PERM_USER|(PERM_WRITEABLE));
		ptrFrames += PAGE_SIZE/4;
	}

	shares[IdRet].empty = 0;
	shares[IdRet].isWritable = isWritable;
	strcpy(shares[IdRet].name, shareName);
	shares[IdRet].ownerID = ownerID;
	shares[IdRet].references = 1;
	shares[IdRet].size = size;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	int no_of_frames = size / PAGE_SIZE;
	uint32 perm = shares[getRet].isWritable << 1;
	uint32* ptr1 = (uint32*)virtual_address;
	for(uint32 i = 0 ; i < no_of_frames ; i++)
	{
		struct Frame_Info *fi = get_frame_from_storage(shares[getRet].framesStorage, i);
		map_frame(myenv->env_page_directory, fi, (void*)ptr1, PERM_USER|(PERM_WRITEABLE&perm));
		ptr1 += PAGE_SIZE/4;
	}
	shares[getRet].references++;
	return getRet;
