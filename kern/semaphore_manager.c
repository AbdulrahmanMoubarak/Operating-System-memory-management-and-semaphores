...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	int allRet = allocate_semaphore_object(&MySem);
	if(allRet == E_NO_SEMAPHORE)
	{
		return E_NO_SEMAPHORE;
	}
	else
	{
		strcpy(semaphores[allRet].name ,semaphoreName);
		semaphores[allRet].ownerID = ownerEnvID;
		semaphores[allRet].value = initialValue;
		semaphores[allRet].empty = 0;
		return allRet;
	}
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	if(ret != E_SEMAPHORE_NOT_EXISTS)
	{
		...
		if(semaphores[ret].value <= 0)
		{
			struct Env *env = dequeue(&semaphores[ret].env_queue);
