...
void* malloc(uint32 size)
{
	...
	uint32 rem = size%PAGE_SIZE;
	if(rem != 0)
	{
		no_of_pages++;
	}
	uint32* start_alloc_va = (uint32*)FindAllocationAddress(no_of_pages*PAGE_SIZE);
	if(start_alloc_va == (uint32*)0)
	{
		return NULL;
	}
	else
	{
		UH_Allocation((uint32)start_alloc_va, no_of_pages*PAGE_SIZE);
		sys_allocateMem((uint32)start_alloc_va, no_of_pages);
		return (void*)start_alloc_va;
	}
...
void* smalloc(char *sharedVarName, uint32 size, uint8 isWritable)
{
	...
	UH_Allocation((uint32)start_alloc_va, no_of_pages*PAGE_SIZE);
	int retID = sys_createSharedObject(sharedVarName, no_of_pages*PAGE_SIZE, isWritable, (void*)start_alloc_va);
	if(retID >= 0)
	{
		return (void*)start_alloc_va;
	}
...
void* sget(int32 ownerEnvID, char *sharedVarName)
{
	...
	UH_Allocation((uint32)start_alloc_va, no_of_pages*PAGE_SIZE);
	int retID = sys_getSharedObject(ownerEnvID, sharedVarName, (void*)start_alloc_va);
	if(retID < 0)
		return NULL;
...
void free(void* virtual_address)
{
	...
	while(1==1)
	{
		if(heap_allocations_addresses[i] == (uint32)virtual_address)
		{
			size = heap_allocations_sizes[i];
			break;
		}
		i++;
	}

	int Rsize = ROUNDUP(size, PAGE_SIZE);
	DeAllocation((uint32)virtual_address, Rsize);
	sys_freeMem((uint32)virtual_address, Rsize);
